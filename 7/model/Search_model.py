                            ...
                            for os in self.__request.get_oss():
                                for colour in self.__request.get_colours():
                                    for in_stock in self.__request.get_in_stocks():
                                        for note in Shop.get_notebooks():
                                            if note.get_brand() == brand and note.get_ram() == ram and note.get_rom() == rom and note.get_proc() == proc and note.get_screen_diag() == screen and note.get_os() == os and note.get_colour() == colour and note.get_in_stock() == in_stock and note.get_price() <= self.get_request().get_max_price():
